...
class CreativeComponent(FlaskForm):
    studentID = StringField(label = 'Enter student ID to delete:')
    studentName = StringField(label = 'Enter student name here')
    prefCuisine = RadioField(label = 'Select your preferred cuisine')
    submitted = SubmitField(label = 'Submit')
...
def adbp():
    ...
    if advprogform.validate_on_submit():
        studid = advprogform.studentID.data

        #TODO: ADD IN STORED PROCEDURE + TRIGGER

        return render_template('home.html')
    return render_template('advdbprog.html')
        #cursur.execute()
    
@app.route('/creativecomp.html', methods = ['GET', 'POST'])
def creativecomp():
    creativecompform = CreativeComponent()
    if creativecompform.validate_on_submit():
        studid = creativecomp.studentID.data
        studnm = creativecomp.studentName.data
        studcuispref = creativecomp.prefCuisine.data

        #TODO: ADD IN CREATIVE COMP

        return render_template('home.html')
    return render_template('/creativecomp.html')

if __name__ == "__main__":
    app.run(host='34.66.171.121', port='80')